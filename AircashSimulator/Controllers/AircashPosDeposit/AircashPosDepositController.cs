...
namespace AircashSimulator.Controllers.AircashPosDeposit
{
    ...
    public class AircashPosDepositController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateCashierPayout(CreatePayoutRQ createPayoutRQ)
        {
            var partnerId = new Guid(PartnerIdCashier);
            var userId = new Guid(UserIdCashier);
            var response = await AircashPosDepositService.CreatePayout(partnerId, createPayoutRQ.Amount, createPayoutRQ.PhoneNumber, userId.ToString(), createPayoutRQ.Parameters);
            return Ok(response);
        }
