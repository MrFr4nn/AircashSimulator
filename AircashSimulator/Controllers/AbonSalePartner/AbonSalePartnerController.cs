...
namespace AircashSimulator
{
    ...
    public class AbonSalePartnerController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateCoupon(CreateCouponRequest createCouponRequest)
        {
            ...
            var response=await AbonSalePartnerService.CreateCoupon(createCouponRequest.Value, createCouponRequest.PointOfSaleId, new Guid(createCouponRequest.PartnerId), createCouponRequest.IsoCurrencySymbol, createCouponRequest.PartnerTransactionId, SettingsService.AircashSimulatorPrivateKeyPath, SettingsService.AircashSimulatorPrivateKeyPass, environment, createCouponRequest.ContentType, (int?)createCouponRequest.ContentWidth);
            return Ok(response);
        }
        ...
        {
            var environment = await UserService.GetUserEnvironment(UserContext.GetUserId(User));
            var response = await AbonSalePartnerService.CancelCoupon(cancelCouponRequest.SerialNumber, cancelCouponRequest.PointOfSaleId, new Guid(cancelCouponRequest.PartnerId), cancelCouponRequest.PartnerTransactionId, SettingsService.AircashSimulatorPrivateKeyPath, SettingsService.AircashSimulatorPrivateKeyPass, environment);
            return Ok(response);
        }
